  ...
  <aside class="nav-rail">
    <div class="brand-panel">
      <div class="brand-main">
        <div class="brand-orb">
          <img class="brand-seal" :src="brandAssets.logoMark" alt="LiteCode" width="72" height="72" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="brand-title">LiteCode</p>
          <p class="brand-subtitle">本地智能体工作台</p>
        </div>
      </div>
      <div class="brand-model-row">
        <p class="brand-provider-pill truncate" :title="`${current?.provider || ''} / ${current?.model || ''}`">
          {{ current?.entryLabel || current?.entryName || `${current?.provider || 'provider'} / ${current?.model ||
            'model'}` }}
        </p>
      </div>
    </div>
    ...
    <nav class="rail-actions" aria-label="Primary navigation">
      <router-link v-for="item in navItems" :key="item.name" :to="item.to" class="rail-action"
        :class="{ active: route.name === item.name }">
        <img :src="resolveNavIcon(item.name, route.name === item.name)" :alt="item.name" class="nav-icon" width="32"
          height="32" />
        <div class="nav-label">
