...
<template>
  <div class="composer-shell" :class="{ 'composer-drag-active': dragActive }" @dragenter="onDragEnter"
    ...
    <div v-if="suggestions.length" class="slash-menu">
      ...
      <div v-if="commandSuggestions.length" class="slash-menu-group">
        ...
        <button v-for="command in commandSuggestions" :key="command.id" type="button" class="slash-menu-item"
          :data-kind="command.kind" @click="applySuggestion(command)">
      ...
      <div v-if="skillSuggestions.length" class="slash-menu-group">
        ...
        <button v-for="skill in skillSuggestions" :key="skill.id" type="button" class="slash-menu-item"
          :data-kind="skill.kind" @click="applySuggestion(skill)">
    ...
    <div v-if="drafts.length || uploading.size" class="composer-drafts">
      <AttachmentChip v-for="(d, i) in drafts" :key="d.id" :data="d" removable @remove="removeDraft(i)" />
    ...
    <form class="composer" @submit.prevent="submit" @keydown.esc="modeMenuOpen = false">
      <input ref="fileInput" type="file" multiple :accept="ACCEPT_LIST" class="hidden-file-input"
        @change="onFileInput" />
      ...
      <div class="composer-input-row">
        <div class="composer-textarea-wrap" :class="{ 'has-skill-slash': composerSlashParts }">
          <div v-if="composerSlashParts" ref="highlightLayer" class="composer-highlight-layer" aria-hidden="true">
            <span class="composer-skill-slash">{{ composerSlashParts.token }}</span><span>{{ composerSlashParts.rest
              }}</span>
          </div>
          <textarea ref="input" v-model="value" rows="2" :disabled="props.busy"
            :placeholder="props.busy ? 'AI 正在执行...' : '给 LiteCode 分配一个任务... 输入 / 查看命令；可拖入图片或文档'"
            @focus="modeMenuOpen = false" @input="resize" @scroll="syncHighlightScroll" @keydown="handleKeydown" />
        </div>
      </div>

      <div class="composer-action-row">
        <div class="composer-left-actions">
          <button type="button" class="attach-button" :title="attachTitle" :aria-label="attachTitle"
            :disabled="props.busy" @click="pickFiles">
            <img class="action-icon" :src="actionAssets.attach" alt="" width="24" height="24" />
          </button>

          <button type="button" class="slash-hint-button" title="显示斜杠命令" :disabled="props.busy"
        ...
        <div class="composer-right-actions">
          <div class="mode-picker" @focusout="scheduleModeMenuClose">
            <button type="button" class="mode-button" :data-active="currentMode.value === 'plan'"
            ...
            <div v-if="modeMenuOpen" class="mode-menu">
              ...
              <button v-for="option in modeOptions" :key="option.value" type="button" class="mode-option"
                :data-active="currentMode.value === option.value" @click="selectMode(option.value)">
          ...
          <div v-if="props.contextMax > 0" class="context-ring" tabindex="0" role="status" :aria-label="contextLabel">
            <svg viewBox="0 0 36 36" class="ring-svg">
              <circle class="ring-track" cx="18" cy="18" r="15.915" />
              <circle class="ring-arc" cx="18" cy="18" r="15.915" :stroke="arcColor"
                :stroke-dasharray="`${arcLength} ${100 - arcLength}`" stroke-dashoffset="25" />
            </svg>
          ...
          <button class="send-button" :disabled="sendDisabled" type="submit">
            <img class="action-icon send-icon" :src="props.busy ? actionAssets.statusBusy : actionAssets.send" alt=""
              width="24" height="24" />
