<p align="center">
  <img src="assets/brand/og-cover.png" alt="LiteCode — Developer Agent Workspace" width="780" />
</p>

<h1 align="center">LiteCode · 开发者智能体</h1>
...
> 用户下发任务，主智能体（LiteCode）统筹上下文、工具、记忆与子代理；把任务拆解、执行、校验后回报。
...
LiteCode 内置 MCP Client，可连接外部 MCP 服务器扩展工具能力。
...
主智能体（LiteCode Assistant）服务用户，可派遣执行代理作为子代理处理独立任务——子代理拥有独立 `history` 与受限工具白名单，办完只把摘要返回主上下文。WebUI 会通过 `subagent_*` 事件实时显示子代理的进度（delta、tool_call、result、done / error）。轻量只读/核验子代理默认走当前 entry 的 `secondaryModelId`，写入型 `neiguan_yingzao` 仍走 `mainModelId`；次模型失败时自动升主模型重试一次。
...
<p align="center">
  <img src="assets/avatars/avatar-emperor.png"  width="140" alt="用户"       />&nbsp;&nbsp;
  <img src="assets/avatars/avatar-eunuch.png"   width="140" alt="主智能体"   />&nbsp;&nbsp;
  <img src="assets/avatars/avatar-subagent.png" width="140" alt="执行代理"   />
</p>
