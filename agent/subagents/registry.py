...
_BUILTIN_SPECS: dict[str, dict] = {
    "xiaohuangmen": {
        "description": (
            "轻量探路执行代理。轻量只读, 适合短命令、快速确认、快速排查。"
            "若发现任务变复杂, 应回报主代理改派更匹配身份。"
        ),
        "tool_names": (
...
_DEFAULT_PROMPT = (
    "你是按主代理任务派遣的执行代理。\n"
    "- 使用专业、简洁、面向交付的语气。\n"
    "- 用工具尽快把任务办妥, 最后用一段简短中文向主代理回报。\n"
    "- 只回禀结论与关键信息, 不要复述每一步细节。\n"
    "- 你不能再派遣其他子代理, 所有差事自己跑工具完成。"
)
