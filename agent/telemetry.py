...
Token 用量跟踪层：每次 LLM 调用记录一行到 tokens.jsonl，并提供多维度聚合统计。

记录字段：时间、provider、model、model_role、usage_type、input/output/cache_read/cache_create token 数。
KV Cache 归一化：Anthropic 的 cache_read_input_tokens/cache_creation_input_tokens
和 OpenAI 的 prompt_tokens_details.cached_tokens 都映射到统一字段。
压缩触发判断：TokenTracker.should_compact(上下文窗口, 阈值)。
"""
...
class TokenTracker:
    """Token 账本：逐条写入 tokens.jsonl，同时维护最近一次调用的输入 token 数与压缩判断。"""

    def __init__(self, log_file: Path):
        self.log_file = log_file
        self.log_file.parent.mkdir(parents=True, exist_ok=True)
        # 加载上一次最后一条记录的 input token，用于判断是否需要压缩
